CREATE PROCEDURE [dbo].[spGetBManagerSummary](

@TransactionId			NVARCHAR(max),
@BusinessManagerID		NVARCHAR(max), 
...
--DECLARE @BusinessManagerID NVARCHAR(max)
--SET @BusinessManagerID = '52,48,0'
...
---if table already exist then drop 
IF OBJECT_ID('tempdb..#TempBusinessManager') IS NOT NULL BEGIN
DROP TABLE #TempBusinessManager
END
...
CREATE TABLE #TempBusinessManager(BusinessManagerID INT,
...
INSERT INTO #TempTransactionType
	...
	WHERE S.CountryId = @CountryID
	AND S.IsActive = 1 --and s.id in (65)
	AND S.Id IN (SELECT
			item
		FROM dbo.SplitString(@TransactionTypes, ','))--and s.id in (65)--and s.id in (65)

--Insert Finance Company
--INSERT INTO #TempBusinessManager
--	SELECT
--		T.BusinessManagerID,
--		UPPER(LTRIM(RTRIM(U.FirstName + ' ' + U.LastName))) AS BusinessManagerName
--	FROM [Transaction] T WITH (NOLOCK)
--	INNER JOIN (SELECT
--		item
--	FROM dbo.SplitString(@BusinessManagerID, ',')) b
--		ON b.Item = T.BusinessManagerID
--	INNER JOIN Users U WITH (NOLOCK)
--		ON U.Id = T.BusinessManagerID
--	GROUP BY	T.BusinessManagerID,
--				U.FirstName,
--				U.LastName

--Newly added code by gaenesh daeted 16/06/2017
INSERT INTO #TempBusinessManager
	SELECT
		U.ID AS BusinessManagerID
	   ,UPPER(LTRIM(RTRIM(CONCAT(ISNULL(Title+' ',''),ISNULL(FirstName+' ',''),ISNULL(LastName+' ',''))))) AS BusinessManagerName
...
DECLARE @Unassigned NVARCHAR(max) = (SELECT TOP 1 item FROM dbo.SplitString(@BusinessManagerID, ',') Where item ='0')

IF(@Unassigned = '0')
INSERT INTO #TempBusinessManager VALUES (NULL,  'UNASSIGNED')

--Update #TempBusinessManager Set BusinessManagerName = 'UNASSIGNED' Where BusinessManagerID IS NULL
...
(SELECT
	   ...
	   ,
		--  (Count((CASE WHEN T.IsIncepted=1 THEN 1 ELSE 0 END)) + COUNT(CASE
		--	WHEN T.IsCashTransaction = 0 THEN 1
		--END)) AS 'VehicleSoldCount',
		--COUNT(CASE
		--	WHEN T.IsIncepted = 1 THEN 1
		--END) AS 'VehicleSoldCount',
	   ...
	   ,0 AS Total_APU
	   ,0.00 AS DivideByProductCount
	   ,0 AS IsInsuranceType

	FROM ViewTransaction AS T WITH (NOLOCK)
	INNER JOIN #TempBusinessManager B
		ON ISNULL(B.BusinessManagerID,0) = ISNULL(T.BusinessManagerID,0)
...
INNER JOIN (SELECT
	...
	WHERE --insurance.BusinessManagerId IS NOT NULL
	...
	GROUP BY p.producttypeName
			...
			,p.id
			,ps.Id
			,insurance.BusinessManagerID) AS M
	ON ISNULL(T.BusinessManagerID,0) = ISNULL(M.BusinessManagerID,0)
...
SET T.ProductCount = M.ProductCount
   ...
   --,T.ProductRetension = 
	--					CASE WHEN T.ProductSubTypeName ='TRANSACTION FEE' THEN -2000 --(M.retension - T.TransactionFee) 
	--					ELSE M.retension END,
	,T.ProductAPP =
	CASE
...
INNER JOIN (SELECT
	...
	FROM #TempOutput M
	GROUP BY M.BusinessManagerID) AS M
	ON ISNULL(T.BusinessManagerID,0) =ISNULL(M.BusinessManagerID,0)
...
INNER JOIN (SELECT
	...
	AND ISNULL(P.BusinessManagerID,0) = ISNULL(T.BusinessManagerID,0)
...
INNER JOIN (SELECT
	...
	FROM #TempOutput
	GROUP BY BusinessManagerID) M
	ON ISNULL(M.BusinessManagerID,0) = ISNULL(T.BusinessManagerID,0)
...
INNER JOIN (SELECT
	...
	FROM #TempOutput
	GROUP BY  BusinessManagerID) P
	ON ISNULL(P.BusinessManagerID,0) =ISNULL( T.BusinessManagerID,0)
...
(SELECT
	 ...
	 --  ,CASE
	   ...
	   ,T.IsInsuranceType
	   ,ISNULL(T.DivideByProductCount, 0.00) AS DivideByProductCount
	FROM #TempOutput T
	UNION ALL SELECT
		1 AS ColumnGroupID
	   ,M.BusinessManagerID
	   ,M.BusinessManagerName
	   ...
	   ,0 AS Total_APU
	   ,0 AS ProductTypeID
	   ,ISNULL(CONVERT(INT, N.ProductSubTypeID), 0) AS ProductSubTypeID
	...
	--SELECT
	--		*
	--	FROM #TempBusinessManager
	--	WHERE BusinessManagerID NOT IN (SELECT
	--			BusinessManagerID
	--		FROM #TempOutput WHERE BusinessManagerID IS NOT NULL)
		SELECT *
		FROM #TempBusinessManager
		WHERE IsNULL(BusinessManagerID,0) NOT IN (SELECT
			  IsNULL(BusinessManagerID,0)
			FROM #TempOutput)	
...
SELECT
	T.ColumnGroupID
   ,CASE WHEN T.BusinessManagerID IS NULL THEN -1 ELSE T.BusinessManagerID END AS BusinessManagerID
   ,T.BusinessManager
   ,T.VehicleSoldCount
   ,T.VehicleProfit
...
FROM T
ORDER BY  
case when BusinessManagerID is null then 1 else 0 end, BusinessManagerID,
--ISNULL(BusinessManagerID, 99999),
--T.BusinessManagerID,
