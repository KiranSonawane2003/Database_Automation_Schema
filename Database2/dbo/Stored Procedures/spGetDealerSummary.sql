CREATE PROCEDURE [dbo].[spGetDealerSummary](
...
---if table already exist then drop 
IF OBJECT_ID('tempdb..#TempBranch') IS NOT NULL BEGIN
...
CREATE TABLE #TempBranch(BranchID INT,
...
INSERT INTO #TempBranch
	SELECT B.Id, UPPER(G.GroupName)+' : '+UPPER(BranchName) AS BranchName
	FROM Branch B
	INNER JOIN [Group] G ON B.GroupId = G.Id 
			AND B.Id IN(SELECT
			item
...
INNER JOIN (SELECT
	...
	WHERE insurance.BranchId IS NOT NULL
	--AND insurance.IsCashTransaction = 0
	AND insurance.TransactionTypeId IN (SELECT
			item
	...
	GROUP BY p.producttypeName
			...
			,p.id
			,ps.Id
			,insurance.BranchId) AS M
	ON T.BranchID = M.BranchId
...
SELECT
   ...
   ,T.ProductPenetration
   ,T.ProductRetension
 --  ,CASE WHEN T.ProductCount > 0 THEN T.ProductRetension ELSE
	--CASE WHEN T.ProductSubTypeName = 'TRANSACTION FEE' THEN ISNULL(T.ProductRetension - T.TransactionFee,0)  ELSE ISNULL(T.ProductRetension, 0) END  END AS ProductRetension
   ,T.ProductAPP
   ,T.ProductAPU
   ,T.ProductTotalAPU
   ,T.SecondGPAPPT
...
FROM T
ORDER BY T.Branch, T.BranchID, T.ProductTypeName, T.ProductSubTypeName
